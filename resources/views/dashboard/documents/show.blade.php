@extends('dashboard.base')

@section('breadcrumbs')
    {{ Breadcrumbs::render('document_show') }}
@endsection

@section('content')

<div class="container-fluid">
    <div class="animated fadeIn">
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i>@lang('document.docTemplate')</div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-12">
                                <h4>@lang('document.title') : {{ $form->name }}</h4>

                                <div class="row">
                                    <div class="col-lg-12">
                                        <table class="table table-hover form-table">
                                            <thead>
                                                <tr>
                                                    <th>@lang('document.label')</th>
                                                    <th>@lang('document.inputType')</th>
                                                    <th>@lang('document.placeholder')</th>
                                                    <th>@lang('document.required')</th>
                                                    <th>@lang('document.readonly')</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach (json_decode($form->schema) as $item)
                                                <tr>
                                                    <td>{{ $item->label }}</td>
                                                    <td>
                                                        <span class="text-uppercase">{{$item->type}}</span>
                                                    </td>
                                                    <td>
                                                        {{ $item->placeholder }}
                                                    <td>
                                                        <span>{{($item->required  == 'true') ? "Yes" : "No"}}</span>
                                                    </td>
                                                    <td>
                                                        <span>{{($item->readonly == 'true') ? "Yes" : "No"}}</span>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <a href="{{ route('documents.index') }}" class="btn btn-primary">@lang('common.back')</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection


@section('javascript')

@endsection
