...
class MenuController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $menulist->save();
        $request->session()->flash('message', translateMessage('messages.createdMenu'));
    ...
    public function update(Request $request){
        ...
        $menulist->name = $request->input('name');
        $menulist->save();
        $request->session()->flash('message',translateMessage('messages.updateMenu'));
    ...
    */
    ...
    public function delete(Request $request){
        ...
        if(!empty($menus)){
            $request->session()->flash('message',translateMessage('messages.cannotDeleteMenu') );
            $request->session()->flash('back', 'menu.menu.index');
            return view('dashboard.shared.universal-info');
        }else{
            Menulist::where('id', '=', $request->input('id'))->delete();
            $request->session()->flash('message',translateMessage('messages.deleteMenu') );
            $request->session()->flash('back', 'menu.menu.index');
            return view('dashboard.shared.universal-info');
        }
    }
